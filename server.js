...
// ---------- AUTH & ADMIN (shortened for brevity) ----------
...
// ========== PAIRING ENGINE – RETRY & ROBUST ==========
const codeStore = {};
const pairingState = {};

async function startPairing(uid, phoneNumber, retries = 0) {
  if (codeStore[uid] && codeStore[uid] !== 'ERROR') return codeStore[uid];
  if (pairingState[uid]?.inProgress) return null;

  pairingState[uid] = { status: 'pending', phone: phoneNumber, inProgress: true, retries };
  ...
  try {
    ...
    // Handle connection close with retry
    sock.ev.on('connection.update', (update) => {
      if (update.connection === 'close') {
        const reason = update.lastDisconnect?.error?.output?.statusCode;
        console.log(`[${uid}] Connection closed. Reason: ${reason}`);
        if (pairingState[uid]) {
          pairingState[uid].status = 'error';
          pairingState[uid].inProgress = false;
        }
        // Retry if not a fatal error and retries < 3
        if (reason !== 401 && retries < 3) {
          console.log(`[${uid}] Retrying pairing (attempt ${retries+1})...`);
          setTimeout(() => {
            startPairing(uid, phoneNumber, retries + 1);
          }, 5000);
        }
      }
    });
    ...
    console.log(`[${uid}] Requesting pairing code for ${cleanNumber}... (attempt ${retries+1})`);
    ...
    if (retries < 3) {
      console.log(`[${uid}] Retrying pairing (attempt ${retries+2})...`);
      setTimeout(() => {
        startPairing(uid, phoneNumber, retries + 1);
      }, 5000);
    } else {
      // After 3 retries, fallback to mock after 20 seconds
      setTimeout(() => {
        if (!codeStore[uid] || codeStore[uid] === 'ERROR') {
          const mockCode = Math.floor(100000 + Math.random() * 900000).toString();
          console.log(`[${uid}] ⚠️ Real pairing failed after retries, using mock code: ${mockCode}`);
