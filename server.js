...
// ----- 🔥 FIX: Polyfill crypto for Baileys -----
...
const codeStore = {};
const pairingState = {}; // uid -> { status, phone, inProgress? }

async function startPairing(uid, phoneNumber) {
  // If a code already exists and is valid, return it
  if (codeStore[uid] && codeStore[uid] !== 'ERROR') return codeStore[uid];

  // Prevent multiple simultaneous attempts for the same UID
  if (pairingState[uid]?.inProgress) {
    console.log(`[${uid}] Pairing already in progress, skipping.`);
    return null;
  }
  ...
  try {
    ...
    // Schedule a mock fallback if real pairing fails
    setTimeout(() => {
...
app.post('/api/pair', async (req, res) => {
  ...
  // If we already have a valid code, return it immediately
  if (codeStore[uid] && codeStore[uid] !== 'ERROR') {
    return res.json({ code: codeStore[uid] });
  }

  // If pairing is already in progress, don't start another one
  if (pairingState[uid]?.inProgress) {
...
app.listen(PORT, () => {
  ...
  console.log(`⚠️  If real pairing fails, a mock code will appear after 15s.`);
});
